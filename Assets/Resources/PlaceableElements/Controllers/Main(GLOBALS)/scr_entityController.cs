...
public class scr_entityController : MonoBehaviour
{
    ...
    public void entityControllerAwake(scr_levelController _LevelController)
    {
        ...
        {
            entityControllerIDScript = gameObject.AddComponent<IDScript>();
    ...
    void onEntityActivation(GameObject currGameObject, bool isMainChar, bool isNewCamTarget)
    {
        if(isMainChar)
        {
            currMainCharScr.gameObject.SetActive(false); //While the burden of deactivating falls to the character scripts, its a good check to do
