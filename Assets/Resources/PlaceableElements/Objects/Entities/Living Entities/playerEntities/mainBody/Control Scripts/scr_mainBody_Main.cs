...
public class scr_mainBody_Main : scr_BaseEntity_Main
{
    ...
    [SerializeField] private const int maxArrowSegments = 4;
    [SerializeField] private GameObject arrowHeadTemplate;
    [SerializeField] private GameObject arrowSegmentTemplate;
    public void arrowCompInitializer()
    {
        arrowComp = gameObject.AddComponent<scr_ArrowCreator_Component>();
        arrowComp.instantiateComponent(maxArrowSegments, arrowSegmentTemplate, arrowHeadTemplate, gameController);
    }
    ...
    void hatThrowLogic()
    {
        ...
        //Therefore, we subtract half the screen so if its directly in the center, it isnt pointing anywhere
        if(isPreppingHat)
        {
            triggerEntitySlowEvent(.33f, .05f);
    ...
    {   //If currently bonked or has an active bonk then
        ...
        {
            if(Math.Abs(velocity.x) >= sprintingStateThreshold * maxVelocity_X)
                characterState = GLOBAL_VARS.CharacterStates.prepAbility3;
            else if (Math.Abs(velocity.x) >= runningStateThreshold * maxVelocity_X)
                characterState = GLOBAL_VARS.CharacterStates.prepAbility2;
            else
                characterState = GLOBAL_VARS.CharacterStates.prepAbility1;
        }
        else if (releasingHat)
        {
